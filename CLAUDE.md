...
O Handbook é um submódulo git deste repositório — não é editado a partir daqui. Alterações de regra, padrão ou arquitetura devem ser propostas no repositório do Handbook (ver `PB-004 - Como Escrever um ADR.md`). Só depois de aprovadas e publicadas lá, este repositório adota a atualização:

```bash
git submodule update --remote docs/handbook
```
